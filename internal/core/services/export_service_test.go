...
func TestCreateCSVExport_VatTemplate(t *testing.T) {
	...
	imageRepo := &stubImageRepo{
		listFn: func(ctx context.Context, u, g uuid.UUID, l, o int) ([]domain.ReceiptImage, error) {
			return []domain.ReceiptImage{
				{ID: uuid.New(), OriginalFilename: "receipt1.jpg"},
			}, nil
		},
	}
	...
	if len(lines) != 2 {
		t.Fatalf("Expected 2 lines for 2 items, got %d", len(lines))
	}
	
	// Check first item (Taxable)
	// G, G, 1, 12345, King Burger, 15/05/2026, MAC001, FS789, , , 2, 100, 200, 30, 230.00
	cols1 := strings.Split(lines[0], ",")
	if cols1[0] != "G" { t.Errorf("Expected G, got %s", cols1[0]) }
	if cols1[1] != "G" { t.Errorf("Expected Gregorian (G), got %s", cols1[1]) }
	if cols1[2] != "1" { t.Errorf("Expected Taxable (1), got %s", cols1[2]) }
	...
	// G, G, 6, 12345, King Burger, 15/05/2026, MAC001, FS789, , , 1, 50, 50, NOTAXBL, 50.00
	cols2 := strings.Split(lines[1], ",")
	if cols2[2] != "6" { t.Errorf("Expected Exempt (6), got %s", cols2[2]) }
	if cols2[13] != "NOTAXBL" { t.Errorf("Expected NOTAXBL, got %s", cols2[13]) }
}
